 ...
 */
router.get('/users', requireAuth, requireAdmin, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    // Get all users
 ...
 */
router.get('/stats', requireAuth, requireAdmin, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const [userCount] = await query('SELECT COUNT(*) as count FROM users');
    const [personaCount] = await query('SELECT COUNT(*) as count FROM personas');
    const [packageCount] = await query('SELECT COUNT(DISTINCT namespace || \'.\' || name) as count FROM packages');
 ...
 */
router.patch('/users/:userId/admin', requireAuth, requireAdmin, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
 ...
 */
router.delete('/users/:userId', requireAuth, requireAdmin, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const { userId } = req.params;
